...
import { Stack, IconButton as MUIIconButton, Box } from '@mui/material';
...
export const IconSelectionControls = () => {
  ...
  const shouldShowFilteredGrid = filter.trim() !== '';
  ...
  return (
    ...
    >
      {shouldShowFilteredGrid && (
        <Section key={`filtered-${filter}`}>
          <IconGrid icons={filteredIcons || []} onMouseDown={onMouseDown} />
        </Section>
      )}
      {!shouldShowFilteredGrid && (
        <Section key="all-icons">
